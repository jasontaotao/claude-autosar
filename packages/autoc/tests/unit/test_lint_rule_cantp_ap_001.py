"""v2.4.1 — CANTP-AP-001 单测。

CanTpChannel with invalid N-PDU size → ERROR。
"""
...
class TestCanTpAp001Rule:
    def test_stub_no_violation(self) -> None:
        """v1 stub：不 yield。"""
        data = _mk_data()
        assert list(CanTpAp001Rule().check(data)) == []

    def test_with_channel_param_no_violation(self) -> None:
        """有 channel 参数但数据不足 → 不报。"""
        data = _mk_data(
            key_params=({"name": "CanTpChannel", "value": "Ch1"},)
        )
        assert list(CanTpAp001Rule().check(data)) == []

    def test_rule_metadata(self) -> None:
        rule = CanTpAp001Rule()
        assert rule.rule_id == "CANTP-AP-001"
