...
```
packages/plugin/plugins/claude-autosar/
├── .claude-plugin/
│   └── plugin.json          # manifest
├── .mcp.json                # MCP server 配置
├── agents/                  # 子 Agent（sub-agent dispatch）
├── commands/                # /claude-autosar:xxx 斜杠命令
├── skills/                  # 自动加载的领域知识
├── hooks/                   # 事件触发脚本
│   ├── hooks.json
│   └── *.py
...
```bash
mkdir -p packages/plugin/plugins/claude-autosar/skills/my-new-skill
```
...
command 是用户可调用的 `/claude-autosar:xxx` 斜杠命令。

```bash
# 创建文件
touch packages/plugin/plugins/claude-autosar/commands/my-new.md
```
...
# /claude-autosar:my-new

详细说明。

## 用法
\`\`\`
/claude-autosar:my-new --arg1 <value>
\`\`\`
...
## 示例
\`\`\`
/claude-autosar:my-new --arg1 foo
\`\`\`
```
...
- 命令名格式 `kebab-case.md`（不带 claude-autosar: 前缀）
- 内部命令靠 Bash 委派到 `claude-autosar` Python CLI
...
HOOKS_DIR = Path(__file__).resolve().parent.parent / "plugins" / "claude-autosar" / "hooks"
...
```bash
# ruff
ruff check packages/plugin/plugins/claude-autosar/hooks/my_new_hook.py packages/plugin/tests/test_my_new_hook.py

# mypy strict
mypy --strict packages/plugin/plugins/claude-autosar/hooks/my_new_hook.py

# black + isort
black packages/plugin/plugins/claude-autosar/hooks/my_new_hook.py packages/plugin/tests/test_my_new_hook.py
isort packages/plugin/plugins/claude-autosar/hooks/my_new_hook.py packages/plugin/tests/test_my_new_hook.py
...
```bash
touch packages/plugin/plugins/claude-autosar/agents/my-new.md
```
...
MCP 工具是给 Claude Code 子 Agent 调用的 Python 函数。在 `packages/autoc/src/claude_autosar/cli/mcp_server.py` 加：
...
- 复用 mcp__claude-autosar__arxml_validate 工具
...
claude --plugin-dir ./packages/plugin/plugins/claude-autosar --debug 2>&1 | grep -i hook
```
...
- [ ] 现有 claude-autosar 测试没破：`pytest packages/autoc/tests/`
