...
行为:
  ...
  3. 拼一段项目摘要 + claude-autosar 用法提示，注入 Claude 上下文
"""
...
def _build_context(cwd: Path) -> str:
    ...
    if eb:
        parts.append(
            f"- 检测到 EB tresos 工程（{eb}）。用 `/claude-autosar:eb-save` 或 "
            f"`claude-autosar eb save --module <Module> --param <path>=<value>` 改参。"
        )
    if davinci:
        parts.append(
            f"- 检测到 DaVinci 工程（{davinci}）。用 `/claude-autosar:davinci-verify` 或 "
            f"`claude-autosar davinci save --module <Module> --param <path>=<value>` 改参。"
        )
    if arxmls:
        top = ", ".join(arxmls[:5])
        ...
        parts.append(
            ...
            f"用 `/claude-autosar:arxml-validate <path>` 校验。"
        )
    if not (eb or davinci or arxmls):
        parts.append(
            "- 未检测到 EB / DaVinci / ARXML 工程。如需手动指定 ARXML："
            "`/claude-autosar:arxml-validate <path>`。"
        )

    parts.append(
        "\n- 子 Agent `claude-autosar:bsw-config` 可处理自然语言改参请求。"
        "\n- 所有改参自动写入 `~/.claude-autosar/agent/sessions/.current`；"
        "用 `/claude-autosar:session-tree show latest` 查看历史。"
        "\n- 当前 session 导出 HTML：`/claude-autosar:export --output report.html`。"
    )
    return "\n".join(parts)
