...
`claude-autosar eb save --module <Module> --param <path>=<value>` 内部流程：
...
## EB tresos → claude-autosar CLI 映射

| tresos_cmd | claude-autosar CLI | 说明 |
|------------|-----------|------|
| `--validate` | `claude-autosar eb verify` | 仅校验，不改文件 |
| `--autocalc` | `claude-autosar eb autocalc` | 触发衍生参数重算 |
| `--generate` | `claude-autosar eb save --generate` | 改 + 生成 |
