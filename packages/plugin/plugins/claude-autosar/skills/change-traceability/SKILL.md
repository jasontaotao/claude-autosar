...
- 每次 `bsw_write` 记录到 `~/.claude-autosar/agent/sessions/<session_id>.jsonl`
...
`claude-autosar log --view timeline --session latest`：
...
`claude-autosar log --view by-url`：
...
```python
# 列所有 session
from claude_autosar.core.session.store import SessionStore, list_session_ids
