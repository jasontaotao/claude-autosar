...
name: export
description: |
  导出会话为自包含 HTML。底层调用 `autoc export --output <path>`。
  用法：`/autoc:export --output <path-to-html> [--session <id|latest>]`
allowed-tools: Bash
---
...
/autoc:export --output report.html --session latest
/autoc:export --output report.html --session 20260611-143000-abc123
```
...
```
> 帮我导出这次会话为 HTML
> /autoc:export --output ./reports/sprint5.html
...
- 有可导出的 session（运行过 `autoc eb save` / `autoc davinci save`）
- 目标目录可写
