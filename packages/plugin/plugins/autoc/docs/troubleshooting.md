...
**症状**：`/autoc:eb-save` 显示 "Unknown command"。
...
# 3. 验证 commands
ls packages/plugin/plugins/autoc/commands/
# 应有 7 个 .md 文件
...
```bash
# 1. 看完整错误
autoc eb save --module Mcu --param Mcu/Clock0/ClockFreq=80000000 --verify --verbose

# 2. 读 schema 找约束
cat $TRESOS_HOME/plugins/Mcu_TS_T40D34M30I0R0/resources/Mcu_BSWMD.arxml | grep ClockSrc

# 3. 一次性改多个联动参数
/autoc:eb-save --module Mcu --param Mcu/Clock0/ClockFreq=80000000 --param Mcu/Clock0/ClockSrc=2
```
...
# 看 hook 源码：packages/plugin/plugins/autoc/hooks/pretooluse_arxml_guard.py
```
...
> 改 ARXML 用 `MultiEdit` 是真的绕过，但改完后建议手动跑 `autoc arxml validate` 校验。
...
**原因**：`~/.autoc/agent/sessions/` 不可写。

**解决**：

```bash
# 修复权限
chmod -R u+w ~/.autoc/agent/sessions/
...
echo '{"cwd": "/path/to/proj"}' | \
    python packages/plugin/plugins/autoc/hooks/sessionstart_detect_project.py
```
