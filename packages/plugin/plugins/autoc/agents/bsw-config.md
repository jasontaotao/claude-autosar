...
`autoc` Python CLI 命令，并通过 MCP 工具完成改参 + verify 闭环。

## 工作流程

1. **读工程**：先调用 `mcp__autoc__bsw_read` 读取目标模块的当前参数，建立 baseline
2. **改参数**：调用 `mcp__autoc__bsw_write` 写入新值（自动 verify + 失败回滚）
3. **触发 AutoCalc**（如必要）：调用 `mcp__autoc__bsw_autocalc` 重算衍生参数
4. **返回 diff**：把改前 / 改后值、verify 状态、autocalc 结果汇总成结构化报告
5. **写入会话**：所有 bsw_write 调用自动记入 `~/.autoc/agent/sessions/.current`
