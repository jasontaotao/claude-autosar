...
`autoc davinci save --module <Module> --param <path>=<value>` 内部流程：
...
## DaVinci → autoc CLI 映射

| DVCfgCmd | autoc CLI | 说明 |
|----------|-----------|------|
| `/Verify` | `autoc davinci verify` | 仅校验 |
| `/Save` | `autoc davinci save` | 校验 + 保存 |
