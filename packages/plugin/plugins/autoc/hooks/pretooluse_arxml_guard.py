...
触发条件:
  ...
  (MultiEdit 不在本 hook 范围：增量多 edit 无法预测最终内容，单独走 autoc arxml validate)

行为:
  1. 解析 stdin 事件
  2. 提取 tool_name 与 file_path
  3. 如匹配 ARXML：解析新内容（Write 用 content；Edit 用 new_string）
  4. 内容 > 5MB 直接拒绝（防 OOM，让用户走 autoc arxml validate）
...
# 仅依赖标准库 + lxml（lxml 是 autoc 项目级依赖）
try:
...
# 5MB 硬上限：超过此大小 hook 拒绝（防 OOM），让用户走 `autoc arxml validate`
...
def _validate_arxml(content: str) -> str | None:
    ...
    if len(content.encode("utf-8")) > _MAX_INLINE_BYTES:
        return (
            ...
            f"请用 `autoc arxml validate <path>` 验证"
        )
    try:
