#!/usr/bin/env python3
"""PostToolUse hook: 写完 .prefs/*.xdm 后自动调 `autoc eb verify` 触发 verify。
...
行为:
  ...
  3. subprocess.run `autoc eb verify --module <module>`，timeout 25s
...
def _run_verify(module: str) -> tuple[int, str, str]:
    """执行 `autoc eb verify --module <module>`；返回 (rc, stdout, stderr)。"""
    autoc = shutil.which("autoc")
    if autoc is None:
        return (
            127,
            "",
            "`autoc` CLI 不在 PATH；安装：`pip install -e ../autoc[dev]`",
        )
    try:
        proc = subprocess.run(
            [autoc, "eb", "verify", "--module", module, "--adapter", "stub"],
            capture_output=True,
            text=True,
            timeout=25,
            check=False,
        ...
        return (124, "", "autoc eb verify 超时（>25s）")
...
def main() -> int:
    try:
        ...
        if rc == 0:
            ctx = (
                f"已对 {module}.xdm 跑 `autoc eb verify`（stub adapter）：通过。\n"
            ...
            ctx = f"`autoc eb verify --module {module}` 失败（rc={rc}）：\n{tail}"
